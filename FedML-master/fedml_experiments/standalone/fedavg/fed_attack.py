...
from fedml_experiments.standalone.fedavg.fedavg_api import FedAvgAPI
...
from fedml_experiments.standalone.fedavg.my_model_trainer_classification import MyModelTrainer
...
def add_args(parser):
    ...
    parser.add_argument('--ci', type=int, default=0,
                        help='CI')
    parser.add_argument('--defend_type', type=str, default='fedavg', help='Defend_type')
    parser.add_argument('--attacker_knowlege', type=str, default='a_little')
    parser.add_argument('--attack_type', type=str, default='model_attack')
    parser.add_argument('--attacker_num', type=int, default=3)
    parser.add_argument('--sample_method',type=str,default='cross_silo')
    parser.add_argument('--norm_bound',type=float,default=1.0)
    parser.add_argument('--stddev',type=float,default=1.0)
    parser.add_argument('--defend_module',type=str,default='AVG')
    return parser
...
def load_data(args, dataset_name):
    ...
    train_labels_type_dic = train_data_local_dict
    aided_dataset = test_data_local_dict
    global_test_aided_data = test_data_local_dict
    dataset = [train_data_num, test_data_num, train_data_global, test_data_global,
               train_data_local_num_dict, train_data_local_dict, train_labels_type_dic, test_data_local_dict,
               aided_dataset, global_test_aided_data, class_num]
    return dataset
...
if __name__ == "__main__":
    ...
    wandb.init(project="fedavg", entity="wyc",
        config=args
    )
    ...
    model = create_model(args, model_name=args.model, output_dim=dataset[10])
    #model=mynet.ResNet50()
