...
def add_args(parser):
    ...
    parser.add_argument('--ci', type=int, default=0,
                        help='CI')
    parser.add_argument('--outputdim', type=int, default=2)
    return parser
...
if __name__ == "__main__":
    ...
    # load data
    train_set=myloader.TrainDataset()
    test_set=myloader.TestDataset()
    dataset= myloader.load_fed_data(args.client_num_in_total,train_set,test_set,args.batch_size)

    # create model.
    # Note if the model is DNN (e.g., ResNet), the training will be very slow.
    # In this case, please use our FedML distributed version (./fedml_experiments/distributed_fedavg)
    model = Unet_model.U_Net().to(device)
