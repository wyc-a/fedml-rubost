...
if __name__ == '__main__':
    ...
    train, test = loan_load_two_party_data(data_dir)  #按路径取出数据集
    Xa_train, Xb_train, y_train = train  #数据集分成三份
    Xa_test, Xb_test, y_test = test      #测试集分成三份

    batch_size = 256
    epoch = 100
    lr = 0.01

    Xa_train, Xb_train, y_train = shuffle(Xa_train, Xb_train, y_train)  #随机打乱
